...
This document outlines the coding style guidelines for Go projects, based on
Google's Go Style Guide with project-specific additions and modifications.
...
- Receiver variable names must be:
  - Short (typically 1-3 characters)
...
- Use `gofmt` or `gofumpt` to format all Go code
...
This ensures code uses modern Go idioms and stays compatible with the Go version
specified in `go.mod`.

### govulncheck

Use `govulncheck` to check for known vulnerabilities in dependencies:
...
- Use features available in the Go version specified in `go.mod`
