...
// TimerStateFromString parses a timer state from its textual representation.
// It returns [TimerStateInvalid] when the input does not match a known state.
...
// AfterFunc creates a new Timer with the given duration and callback.
...
// FromTime creates a new Timer with the given start time and duration.
...
func newInvalidTimerSnapshotError(msg string) error {
	return fmt.Errorf("%w: %s", ErrInvalidTimerSnapshot, msg)
}

// Validate returns an error if the snapshot is nil or contains an invalid state,
// zero start time, or an inconsistent stop time.
func (s *TimerSnapshot) Validate() error {
	if s == nil {
		return newInvalidTimerSnapshotError("nil snapshot")
	}

	if !s.State.IsValid() {
		return newInvalidTimerSnapshotError(fmt.Sprintf("invalid timer state %q", s.State))
	}

	if s.StartTime.IsZero() {
		return newInvalidTimerSnapshotError("start time is zero")
	}

	if !s.StopTime.IsZero() && s.StopTime.Before(s.StartTime) {
		return newInvalidTimerSnapshotError(fmt.Sprintf("stop time %q is before start time %q", s.StopTime, s.StartTime))
	}
...
// RestoreTimer recreates a Timer from its snapshot.
